...
public class getoptioncategory {
    ...
    public HashMap process(HashMap input) {
        ...
        Gson gson = new Gson();
        ArrayList<category> listcategoryname = dp.getoptioncategory(input.get(FieldParameterMatapos.brand_id).toString(), input.get(FieldParameterMatapos.outlet_id).toString());
        try {
            if (listcategoryname != null) {
                JsonElement element = gson.toJsonTree(listcategoryname);

                input.put(FieldParameterMatapos.listcategoryname, element);
