...
public class itemreport {
    ...
    private static Logger log = Logger.getLogger(itemreport.class);

    public HashMap process(HashMap input) {
        BackendDBProcess dp = new BackendDBProcess();
        Gson gson = new Gson();
        ArrayList<itemreports> ListMerchant = dp.getlistitemreport(input.get(FieldParameterMatapos.brand_id).toString(), input.get(FieldParameterMatapos.outlet_id).toString(), input.get(FieldParameterMatapos.tanggal_from).toString(), input.get(FieldParameterMatapos.tanggal_to).toString());
        try {
            if (ListMerchant != null) {
                JsonElement element = gson.toJsonTree(ListMerchant);

                input.put(FieldParameterMatapos.listitemreport, element);
